...
API_TOKEN = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJleHAiOjE3MjM4MTgyMzgsImlhdCI6MTcyMzY1MDIzOCwiaXNzIjoiL2FwaS92MS9hdXRoL2Fub255bW91cyIsInN1YiI6IjM2Y2Y5YmViM2IwOTBiNGM5N2RmYzljMmU2ZTQ3NWUwZjQxNzRhZjhhMmJiZDljZWM2ODAwNThmNTQ5ZGMyMGUiLCJ0eXBlIjoxMH0.k3tZt3m6ky6vMq3KDMamikXBNXApZJ7dQPZgLH5cj1o"
...
def test_search_books_rus():
    ...
    response = requests.get(f"{BASE_URL_2}/search/product?phrase=Лолита", headers=headers)
    assert response.status_code == 200, f"Ожидался статус-код 200, но получен {response.status_code}"
...
def test_search_books_eng():
    ...
    response = requests.get(f"{BASE_URL_2}/search/product?phrase=Lolita", headers=headers)
    assert response.status_code == 200, f"Ожидался статус-код 200, но получен {response.status_code}"
...
def test_get_category_books():
        ...
        response = requests.get(f"{BASE_URL_2}/products?forceFilters[categories]=110003", headers=headers)
        assert response.status_code == 200, f"Ожидался статус-код 200, но получен {response.status_code}"
...
def test_get_books_by_author():      
        ...
        response = requests.get(f"{BASE_URL_2}/products/facet?filters[authors]=568868", headers=headers)
        assert response.status_code == 200, f"Ожидался статус-код 200, но получен {response.status_code}" 
